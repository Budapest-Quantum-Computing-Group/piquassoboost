...
tbb::tick_count t4 = tbb::tick_count::now();
    ...
    pic::Complex16 hafnian_eigen = hafnian_calculator_powertrace.calculate();
...
std::cout << (t1-t0).seconds() << " " <<(t3-t2).seconds() << " " << (t5-t4).seconds() << std::endl;
    ...
    std::cout << "the calculated hafnian with the powertrace method: " << hafnian_eigen << std::endl;
    ...
    assert( std::abs(hafnian_eigen-hafnian_expected)<1e-13 );
