...
/// Definition of the LAPACKE_dgetrf function from LAPACKE to calculate the LU
/// decomposition of a real matrix
int LAPACKE_dgetrf(int matrix_layout, int n, int m, double *a, int lda,
                   int *ipiv);

/// Definition of the LAPACKE_dgetri function from LAPACKE to calculate the
...
ThresholdMeasurementSubstate::ThresholdMeasurementSubstate(matrix_real &O,
                                                           double Qdet_sqrt_rec)
    : O(O), Qdet_sqrt_rec(Qdet_sqrt_rec) {}
...
*/
static inline int64_t sum(PicState_int64 &vec) {
...
ThresholdBosonSampling::ThresholdBosonSampling(
    matrix_real &covariance_matrix_in)
  ...
  // Space for storing the threshold measurement specific datas for a sample
  // which are equal in all samples.
  pmfs = std::unordered_map<PicState_int64, double, PicStateHash_int64>();
  
  /// The upper bound of maximaly sampled photons
  upbound = 20;
}
...
@param covariance_matrix The covariance matrix describing the gaussian state
*/
void ThresholdBosonSampling::Update_covariance_matrix(
    matrix_real &covariance_matrix_in) {
  covariance_matrix = covariance_matrix_in;
}

/**
@brief Call to determine the resultant state after traversing through linear
interferometer.
@param samples_number The number of shots for which the output should be
determined
@return Returns with the resultant state after traversing through linear
interferometer.
*/
std::vector<PicState_int64> ThresholdBosonSampling::simulate(int samples_number) {
...
void ThresholdBosonSampling::fillSubstates(matrix_real &matrix,
                                           int mode_number) {

  // initialize the substates vector with mode_number+1 elements for the empty
  // modes ... all modes
  ...
  for (int mode_idx = 1; mode_idx < mode_number + 1; mode_idx++) {
    ...
    // reduced covariance matrix in reduced gaussian state to the first mode_idx
    // modes get the reduced gaussian state describing the first mode_idx modes
    matrix_real reduced_matrix = reduce(matrix, indices_2_extract);

    // since the determinant can be calculated by LU factorization, which is
    // also necessary to calculate the inverse, we calculate the inverse and the
    // determiant in one shot. Calculate Q matrix defined by \sigma + 0.5 * Id
    ...
    // calculate the Hamilton matrix O defined by Eq. (14) of Ref. Exact
    // simulation of Gaussian boson sampling in polynomial space and exponential
    // time O := 1 - Q^-1
...
*/
PicState_int64 ThresholdBosonSampling::getSample() {

  bool flag = false;
  PicState_int64 output_sample(0);
  do {
    flag = false;
    output_sample = PicState_int64(0);
    output_sample.number_of_photons = 0;

    // probability of the sampled state
    double last_probability = 1.0;

    // The number of modes is equal to number_of_modes (because the covariance
    // matrix contains p,q quadratires) for loop to sample
    // 1,2,3,...number_of_modes modes These samplings depends from each other by
    // the chain rule of probabilites (see Eq. (14) of Ref. Exact simulation of
    // Gaussian boson sampling in polynomial space and exponential time))
    for (size_t mode_idx = 1; mode_idx <= number_of_modes; mode_idx++) {
      ...
      // calculate probabilities whether there are any photons on the mode
      // mode_idx

      // current output variable is used for storing the conditions for the
      // conditional probablities. create array for the new output state
      PicState_int64 current_output0(output_sample.size() + 1, 0);
      memcpy(current_output0.get_data(), output_sample.get_data(),
             output_sample.size() * sizeof(int64_t));
      ...
      // sometimes the probability is negative which is coming from a negative
      // hafnian.
      ...
      memcpy(current_output1.get_data(), output_sample.get_data(),
             output_sample.size() * sizeof(int64_t));
      ...
      memcpy(current_output.get_data(), output_sample.get_data(),
             output_sample.size() * sizeof(int64_t));
      current_output[mode_idx - 1] = chosen_index;
      current_output.number_of_photons =
          output_sample.number_of_photons + chosen_index;

      if (current_output.number_of_photons > upbound) {
        flag = true;
        break;
      } else {
...
/**
@brief Call to calculate the Hamilton matrix A defined by Eq. (14) of Ref. Exact
simulation of Gaussian boson sampling in polynomial space and exponential time.

O = 1 - Qinv

@param Qinv An instace of matrix class conatining the inverse of matrix Q
calculated by method get_Qinv.
@return Returns with the Hamilton matrix A.
*/
matrix_real ThresholdBosonSampling::calc_HamiltonMatrix(matrix_real &Qinv) {
  // multiply by -1 the elements of Qinv and store the result in the
  ...
  for (size_t row_idx = 0; row_idx < O.rows; row_idx++) {
    ...
    for (size_t col_idx = 0; col_idx < O.cols; col_idx++) {
      O[row_idx * O.stride + col_idx] =
          -1.0 * Qinv[row_idx * Qinv.stride + col_idx];
    }
...
/**
@brief Call to calculate the probability associated with observing output state
given by current_output

If the size of the given current_output is smaller then limit_for_using_pmfs
then we use cache for having faster probability calculation. Otherwise we just
calculate the probability.

@param current_output The current conditions for which the conditional
probability is calculated
@return Returns with the calculated probability
*/
double ThresholdBosonSampling::calc_probability_from_cache(
    PicState_int64 &current_output) {
  ...
  if (current_output.size() < limit_for_using_pmfs) {
    ...
    if (pmfs.end() != current_prob_iter) {
      ...
      // Call the normal calc_probability method which does not store the
      // results
...
The calculation is based on Eq. (14) of Ref. Exact simulation of Gaussian boson
sampling in polynomial space and exponential time.
...
ThresholdBosonSampling::calc_probability(PicState_int64 &current_output) {
  ...
  // calculate the normalization factor defined by the square root of the
  ...
  // create Matrix O_S according to the main text below Eq. (14) of Ref. Exact
  // simulation of Gaussian boson sampling in polynomial space and exponential
  // time.
...
/**
@brief Call to create matrix O_S according to the main text below Eq. (14) of
Ref. Exact simulation of Gaussian boson sampling in polynomial space and
exponential time.
@param O Hamilton matrix O
@param current_output The fock representation of the current output for which
the probability is calculated
@return Returns with the O_S matrix
*/
matrix_real ThresholdBosonSampling::create_O_S(matrix_real &O,
                                               PicState_int64 &current_output) {
  ...
  for (size_t idx_output = 0; idx_output < current_output.size();
       idx_output++) {
    ...
    if (current_output[idx_output]) {
      ...
      for (size_t jdx_output = 0; jdx_output < current_output.size();
           jdx_output++) {
      ...
      for (size_t jdx_output = 0; jdx_output < current_output.size();
        ...
        // we inserting element if the current output is 1
        if (current_output[jdx_output]) {
          O_S[row_offset + col_idx_O_S + dim_O_S] =
              O[row_offset_O + jdx_output + dim_O];
          col_idx_O_S++;
        }
      }
...
// TODO: The reduction could be placed to a separate place, since this is not
// specific to this class.
matrix_real ThresholdBosonSampling::reduce(matrix_real &matrix,
                                           PicState_int64 &indices_to_reduce) {

  size_t total_number_of_modes = matrix.rows / 2;

  if (total_number_of_modes == 0) {
    std::cout << "ThresholdBosonSampling::reduce: There is no covariance "
                 "matrix to be reduced. Exiting"
              << std::endl;
    exit(-1);
  }
  ...
  if (number_of_modes == total_number_of_modes) {
    ...
    std::cout << "ThresholdBosonSampling::reduce: The number of modes to be "
                 "extracted is larger than the possible number of modes. "
                 "Exiting"
              << std::endl;
    exit(-1);
  }
  ...
  matrix_real matrix_reduced(number_of_modes * 2,
                             number_of_modes * 2); // the size of the covariance
                                                   // matrix must be the double
  ...
  size_t mode_idx = 0;
  size_t col_range = 1;
  // loop over the col indices to be transformed (indices are stored in
  // attribute modes)
  while (true) {

    // condition to exit the loop: if there are no further columns then we exit
    // the loop
    ...
    while (true) {
      ...
      // condition to exit the loop: if the difference of successive indices is
      // greater than 1, the end of the contiguous memory slice is determined
      if (mode_idx + col_range >= number_of_modes ||
          indices_to_reduce[mode_idx + col_range] -
                  indices_to_reduce[mode_idx + col_range - 1] !=
              1) {
        break;
      } else {
    ...
    // row-wise loop to extract the q quadrature columns from the covariance
    // matrix
    for (size_t mode_row_idx = 0; mode_row_idx < number_of_modes;
         mode_row_idx++) {

      // col-wise extraction of the q quadratures from the covariance matrix
      size_t cov_reduced_offset =
          mode_row_idx * matrix_reduced.stride + mode_idx;
      size_t cov_offset =
          indices_to_reduce[mode_row_idx] * matrix.stride + col_idx;
      memcpy(matrix_reduced_data + cov_reduced_offset, matrix_data + cov_offset,
             col_range * sizeof(double));
    ...
    // row-wise loop to extract the p quadrature columns from the covariance
    // matrix
    for (size_t mode_row_idx = 0; mode_row_idx < number_of_modes;
         mode_row_idx++) {
      ...
      // col-wise extraction of the q quadratures from the covariance matrix
      size_t cov_reduced_offset =
          (mode_row_idx + number_of_modes) * matrix_reduced.stride + mode_idx;
      size_t cov_offset =
          (indices_to_reduce[mode_row_idx] + total_number_of_modes) *
              matrix.stride +
          col_idx;
      memcpy(matrix_reduced_data + cov_reduced_offset, matrix_data + cov_offset,
             col_range * sizeof(double));
    ...
    if (m.size() > 0) {
        memcpy(m_reduced_data + mode_idx, m_data + col_idx,
    col_range*sizeof(Complex16)); // q quadratires memcpy(m_reduced_data +
    mode_idx + number_of_modes, m_data + col_idx + total_number_of_modes,
    col_range*sizeof(Complex16)); // p quadratures
    }
    */
...
/**
@brief Call to calculate the inverse of matrix Q defined by Eq (3) of Ref. arXiv
2010.15595 and the determinant of Q. Since the determinant can be calculated by
LU factorization, which is also necessary to calculate the inverse, we
calculatet the inverse and the determiant in one shot.
@param state An instance of Gaussian state in the Fock representation. (If the
Gaussian state is in quadrature representation, than it is transformed into
Fock-space representation)
@param Qdet The calculated determinant of the matrix Q is stored into this
value.
@return Returns with the Hamilton matrix A.
*/
matrix_real ThresholdBosonSampling::calc_Qinv(matrix_real &matrix,
                                              double &Qdet) {
  ...
  LAPACKE_dgetrf(LAPACK_ROW_MAJOR, Q.rows, Q.cols, Q.get_data(), Q.stride,
                 ipiv);
  ...
  int info =
      LAPACKE_dgetri(LAPACK_ROW_MAJOR, Q.rows, Q.get_data(), Q.stride, ipiv);
  ...
  for (size_t i = 0; i < dim; i++) {
    for (size_t j = 0; j < dim; j++) {
      ...
      for (size_t k = 0; k < dim; k++) {
        product[i * product.stride + j] +=
            Qinv[i * Q.stride + k] * Qcopy[k * Qcopy.stride + j];
      }
    }
  ...
  */
  void 
  ThresholdBosonSampling::set_photon_upper_bound(int upbound_) {
  ...
  */
  int 
  ThresholdBosonSampling::get_photon_upper_bound() {
