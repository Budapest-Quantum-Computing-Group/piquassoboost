...
"""Unit tests for piquassoboost's permanent calculator.

These tests mirror piquasso's test_permanent.py but call the C++ BBFG
permanent calculator directly via piquassoboost, verifying that piquassoboost's
implementation is numerically correct.

Note: piquasso no longer routes through piquassoboost's permanent by default;
this file ensures piquassoboost's own implementation is tested regardless.

BBFGRepeatedPermanentCalculatorDouble(matrix, cols, rows) corresponds to
piquasso's permanent(matrix, cols=cols, rows=rows).
"""
...
def _perm(matrix, cols, rows):
    result = BBFGRepeatedPermanentCalculatorDouble(
        matrix,
...
def test_permanent_trivial_case():
    ...
    input = output = np.ones(1, dtype=np.int64)

    assert np.isclose(_perm(matrix, input, output), 4.2)
...
def test_permanent_zero_input():
    ...
    input = output = np.zeros(3, dtype=np.int64)

    assert np.isclose(_perm(interferometer, input, output), 1.0)
...
def test_permanent_no_repetition():
    ...
    input = output = np.ones(6, dtype=np.int64)

    assert np.isclose(
        _perm(interferometer, input, output),
        0.022227325527358795 + 0.01807052573717885j,
    )


def test_permanent_2_by_2_asymmetric():
    output = np.array([2, 0], dtype=np.int64)
    input = np.array([0, 2], dtype=np.int64)

    interferometer = np.array([[1, 1j], [1, -1j]]) / np.sqrt(2)

    assert np.isclose(_perm(interferometer, input, output), -1)
...
def test_permanent_4_by_4():
    ...
    rows = np.array([3, 0, 0, 1], dtype=np.int64)
    cols = np.array([0, 0, 3, 1], dtype=np.int64)

    assert np.isclose(
        _perm(unitary, cols, rows),
...
def test_permanent_6_by_6():
    ...
    input = np.array([2, 1, 3, 0, 1, 2], dtype=np.int64)
    output = np.array([1, 1, 0, 3, 2, 2], dtype=np.int64)
