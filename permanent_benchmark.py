...
# BBFG Inf repeated permanent calculator
if (dim<=20):
    permanent_Glynn_repeated_calculator = GlynnRepeatedInfPermanentCalculator( Arep, input_state=input_state, output_state=output_state )
    time_Glynn_repeated_InfinitePrecision = 1000000000
    for idx in range(iter_loops):
        start = time.time()   

        permanent_Glynn_repeated_InfinitePrecision = permanent_Glynn_repeated_calculator.calculate()
        time_loc = time.time() - start
        start = time.time()   
       
        if time_Glynn_repeated_InfinitePrecision > time_loc:
            time_Glynn_repeated_InfinitePrecision = time_loc


        

    print( permanent_Glynn_repeated_InfinitePrecision )
...
if (dim<=sum(input_state)):
    print('Time elapsed with infinite precision repeated Glynn: ' + str(time_Glynn_repeated_InfinitePrecision))
