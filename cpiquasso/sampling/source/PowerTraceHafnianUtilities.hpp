...
calc_power_traces(matrix &AZ, size_t pow_max) {
    ...
    // quad precision
        ...
        matrix_type coeffs_labudde32(coeffs_labudde.rows, coeffs_labudde.cols);
        for (size_t idx=0; idx<coeffs_labudde.size(); idx++) {
            coeffs_labudde32[idx].real( (long double)coeffs_labudde[idx].real() );
            coeffs_labudde32[idx].imag( (long double)coeffs_labudde[idx].imag() );
        }

        // calculate the power traces of the matrix AZ using LeVerrier recursion relation
        return powtrace_from_charpoly<matrix_type>(coeffs_labudde32, pow_max);
